...
import eu.kanade.tachiyomi.ui.anime.episode.BaseEpisodeAdapter
...
class RecentAnimeAdapter(val delegate: RecentsInterface) :
    BaseEpisodeAdapter<IFlexible<*>>(delegate) {
    ...
    fun setPreferenceFlows() {
        ...
        uiPreferences.outlineOnCovers().register(false) {
            ...
            (0 until itemCount).forEach { i ->
                (recyclerView.findViewHolderForAdapterPosition(i) as? RecentAnimeHolder)?.updateCards()
            }
        }
    ...
    fun getItemByEpisodeId(id: Long): RecentAnimeItem? {
        return currentItems.find {
            val item = (it as? RecentAnimeItem) ?: return@find false
            return@find id == item.episode.id || id in item.aeh.extraEpisodes.map { ch -> ch.id }
        } as? RecentAnimeItem
    }
    ...
    interface RecentsInterface : GroupedDownloadInterface {
        ...
        fun onSubEpisodeClicked(position: Int, episode: Episode, view: View)
        fun updateExpandedExtraEpisodes(position: Int, expanded: Boolean)
        fun areExtraEpisodesExpanded(position: Int): Boolean
        fun markAsRead(position: Int)
        fun alwaysExpanded(): Boolean
        fun scope(): CoroutineScope
        fun getViewType(): RecentsViewType
        fun onItemLongClick(position: Int, episode: EpisodeHistory): Boolean
    }
